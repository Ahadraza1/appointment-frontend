...
const MyAppointments = () => {
  ...
  const filteredAppointments = appointments.filter(apt => {
    ...
    if (filter === 'upcoming') statusMatch = ['pending', 'approved'].includes(apt.status);
    else if (filter === 'past') statusMatch = ['rejected', 'cancelled', 'completed'].includes(apt.status);
    else if (filter !== 'all') statusMatch = apt.status === filter;

    // 2. Search Filter
    const searchMatch = !searchTerm || apt.serviceId?.name?.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="container">
          <div className="hero-content">
            ...
            <p className="hero-description">
              Manage your professional service schedule and track appointment statuses in real-time.
            </p>
          </div>
        </div>
      </section>
                  ...
                  <div key={appointment._id} className={`appointment-card ${appointment.status}`}>
                    ...
                    <div className="card-details">
                      <div className="service-main">
                        <h3 className="service-name">{appointment.serviceId?.name || 'Service'}</h3>
                    ...
                    <div className="card-actions-wrapper">
                      ...
                      {['pending', 'approved'].includes(appointment.status) && (
                        ...
                        >
                          {cancellingId === appointment._id ? 'Processing...' : 'Cancel'}
                        </button>
                      )}
                    </div>
                  </div>
