...
const SuperAdminLayout = () => {
  ...
  return (
    <div className={`sa-layout ${collapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      {/* Sidebar */}
      <aside className={`sa-sidebar ${sidebarOpen ? 'open' : ''} ${collapsed ? 'collapsed' : ''}`}>
        ...
        <nav className="sa-sidebar-nav">
          ...
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) => `sa-sidebar-nav-link ${isActive ? 'active' : ''}`}
              onClick={() => setSidebarOpen(false)}
        ...
        <div className="sa-sidebar-footer">
          <button className="sa-sidebar-logout-btn" onClick={handleLogout} title="Logout">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="sa-header-right">            
            <div className="sa-profile-dropdown">
              <div className="sa-profile-info">
                <span className="sa-profile-name">{user?.name || 'Super Admin'}</span>
